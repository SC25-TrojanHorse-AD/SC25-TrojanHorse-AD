...
folders = [f for f in os.listdir('.') if os.path.isdir(f) and not f.startswith('superlu_')]
print(folders)
folders = [ 'ex11','gas_sensor', 'shipsec1','para-8', 'inline_1','ldoor']
...
for i, folder in enumerate(folders):
    ...
    try:
        ...
        axes[i].fill_between(time_us_big_nobatch, gflops_big_nobatch, gflops_small_nobatch, color='blue', alpha=0.3,label='PanguLU',zorder=1)
        axes[i].plot(time_us_mid_batch, gflops_mid_batch, color='red',zorder=2)
        axes[i].fill_between(time_us_big_batch, gflops_big_batch, gflops_small_batch, color='red', alpha=0.3,label='PanguLU with Trojan Horse',zorder=1)
        ...
        axes[i].set_ylim(bottom=0, top=gflops_max)
        axes[i].set_xlim(left=0, right=time_max)
        meta = float(f'{time_max / 4:.2f}')
        meta2 = float(f'{time_max / 4*2:.1f}')
        meta4 = float(f'{time_max / 4*4:.1f}')
        ...
        axes[5].set_xticks(ticks=[0,5,10,15])
        axes[5].set_xticklabels([0,5,10,15], fontsize=20)
...
plt.savefig("batch_compare_pangulu.pdf", dpi=300, bbox_inches='tight')  
# plt.show()