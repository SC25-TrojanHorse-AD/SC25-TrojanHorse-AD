...
for i in range(0, len(data['time(us)']), step):
    ...
    geom_mean_value = np.exp(np.mean(np.log(subset['gflops'])))  
    geom_mean_index = subset['gflops'].sub(geom_mean_value).abs().idxmin() 
    ...
    geom_mean_x_values.append(subset.loc[i, 'time(us)'])  
    geom_mean_y_values.append(subset.loc[geom_mean_index, 'gflops'])  
...
plt.plot(geom_mean_x_values, geom_mean_y_values, color='y', label='Geometric Mean Points')
...
max_data_df.to_excel('nobatchMAX.xlsx', index=False)
...
min_data_df.to_excel('nobatchMIN.xlsx', index=False)
...
mean_data_df.to_excel('nobatchMEAN.xlsx', index=False)

print("nobatch---GFlops---SAVE!!!")