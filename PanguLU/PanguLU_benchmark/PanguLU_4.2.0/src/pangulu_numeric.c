...
#else
        CPU_SET(cpu_thread_count_per_node - ((pangu_omp_num_threads * rank + i) % cpu_thread_count_per_node) - 1, &cpuset);
#endif
    }
...
#else
    bind_to_core(sysconf(_SC_NPROCESSORS_ONLN) - ((pangu_omp_num_threads * rank) % cpu_thread_count_per_node) - 1);
#endif
    pangulu_int32_t block_length = block_common->block_length;
    pangulu_int32_t p = block_common->p;
    pangulu_int32_t q = block_common->q;
