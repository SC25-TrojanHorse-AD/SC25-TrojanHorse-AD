...
#else
        CPU_SET(cpu_thread_count_per_node - ((pangulu_omp_num_threads * rank + i) % cpu_thread_count_per_node) - 1, &cpuset);
#endif
    }
...
#else
        bind_to_core(cpu_thread_count_per_node - ((pangulu_omp_num_threads * rank + tid) % cpu_thread_count_per_node) - 1);
#endif
    }
