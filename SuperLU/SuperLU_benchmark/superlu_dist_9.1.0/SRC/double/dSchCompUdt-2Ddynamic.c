...
#ifdef _OPENMP
#pragma omp parallel for firstprivate(iukp, rukp) private(j, tempu, jb, nsupc, ljb, segsize, lead_zero, jj, i) default(shared) schedule(CPU_SCHEDULE_STRATEGY)
#endif
		for (j = jj0; j < nub; ++j)
		{ /* jj0 starts after look-ahead window. */
...
#pragma omp for private(nsupc, ljb, lptr, ib, temp_nbrow, cum_nrow) \
	schedule(dynamic)
	...
	if (Rnbrow > 0 && ldu > 0)
	{ /* There are still blocks remaining ... */
...
#pragma omp for private(j, lb, rukp, iukp, jb, nsupc, ljb, lptr, ib, temp_nbrow, cum_nrow) \
	schedule(dynamic)
#else /* not use _OPENMP */
		thread_id = 0;
		int *indirect_thread = indirect;
