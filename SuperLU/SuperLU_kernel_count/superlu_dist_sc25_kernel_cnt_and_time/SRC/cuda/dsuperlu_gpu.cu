...
extern "C"{	
	...
	void trojan_horse_init(int_t nsupers){
		cudaMalloc(&th_dense_browidx_d, sizeof(unsigned long long)*nsupers);
		cudaMemset(th_dense_browidx_d, 0xFF, sizeof(unsigned long long)*nsupers);
		u_colidx_h = (int_t*)malloc(sizeof(int_t)*(2*nsupers+1));
		cudaMalloc(&u_colidx_d, sizeof(int_t)*(2*nsupers+1));
