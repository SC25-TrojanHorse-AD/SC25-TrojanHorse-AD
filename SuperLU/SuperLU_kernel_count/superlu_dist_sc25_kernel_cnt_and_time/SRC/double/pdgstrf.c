...
void trojan_horse_batched_kernel(void* _param){
    ...
    // SSSSM, update L
    // timer_start(&tv_start1);
    trojan_horse_grouped_batched_scatter_l(
        nsupers,
        nrow_effective,
        ncol_effective,
        nsupc,
    ...
    if((level + 1) < nsupers && ((level + 1) % LYD_PGRID_Q != iam % LYD_PGRID_Q) && Llu->Lrowind_bc_ptr[LBj((level + 1), grid_glo)]){
        gpuMemcpyAsync(Llu->Lnzval_bc_ptr[LBj((level + 1), grid_glo)], 
            Llu->l_bcol_val_hd[level + 1], 
            sizeof(double) * Llu->Lrowind_bc_ptr[LBj((level + 1), grid_glo)][1] * SuperSize(level + 1), 
    ...
    gpu_kernel_time += timer_end(&tv_start1);
    kernel_count += 2;
}
...
// #define LYD_PGRID_RANK(brow, bcol) (brow * LYD_PGRID_Q + bcol)
    ...
    bind_to_core(2*(2*iam));
    Thread_Pool* trojan_horse_thread_pool = create_pool(1, 2*(2*iam+1));
    //bind_to_core(sysconf(_SC_NPROCESSORS_ONLN) - 1 - 2*iam);
    //Thread_Pool* trojan_horse_thread_pool = create_pool(1, sysconf(_SC_NPROCESSORS_ONLN) - 1 - (2*iam+1));
    ...
    for(int i=0;i<nproc;i++){
        if(iam == i){
            ...
            printf("%lld kernel_time = %lf s, kernel_count = %lld\n", iam, gpu_kernel_time+time_getrf_kernel+time_gessm_kernel+time_getrf_kernel, kernel_count);
            fflush(stdout);
        }
        usleep(100);
        MPI_Barrier(MPI_COMM_WORLD);
