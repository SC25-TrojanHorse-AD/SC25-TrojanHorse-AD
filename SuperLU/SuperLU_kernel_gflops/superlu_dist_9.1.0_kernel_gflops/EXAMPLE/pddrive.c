...
int main(int argc, char *argv[])
{
    ...
    if (!iam)
    {
        ...
        char file_path[120];
        sprintf(file_path, "superlu_910_line_%s.csv", mtx_name_glo);
    ...
    int cpu_thread_count_per_node = sysconf(_SC_NPROCESSORS_ONLN);
    bind_to_core(cpu_thread_count_per_node - 1 - iam);
