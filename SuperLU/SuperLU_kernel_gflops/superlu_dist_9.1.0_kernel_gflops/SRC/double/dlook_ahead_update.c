...
#pragma omp parallel for \
    ...
    timer_start(&start);
        ...
        long long cub = temp_nbrow * ncols * ldu;
        long long memsize = sizeof(double) * ((temp_nbrow * ncols) + (ncols * ldu) + (temp_nbrow * ldu));
        double gflops = 2 * cub / elapsed_time / 1e9;
        double gBps = memsize / elapsed_time / 1e9;
        struct timeval timestamp;
        gettimeofday(&timestamp, NULL);
        // timestamp, gflops, GB/s, cub, elapsed_time
        if(elapsed_time > 0 && cub != 0){
